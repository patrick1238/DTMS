...
public class Exporter {
    ...
    public static void exportCaseOverview() {
        ...
        if (selectedDirectory == null) {
            ...
            String output = "CaseNumber,Diagnose,Available Stainings\n";
            for (ArrayList images : caseIds.values()) {
                if (!images.isEmpty()) {
                    ImageObjectGeneral i = (ImageObjectGeneral) images.get(0);
                    output = output + i.getCaseNumber() + ",\"" + i.getDiagnose() + "\",\"";
                    for (Object o : images) {
                        ImageObjectGeneral img = (ImageObjectGeneral) o;
                        if (img.getPrimaryStaining().equals("") && img.getSecondaryStaining().equals("")) {
                            output = output + "(negCol)";
                        } else if (img.getSecondaryStaining().equals("")) {
                            output = output + "(" + img.getPrimaryStaining() + ")";
                        } else {
                            output = output + "(" + img.getPrimaryStaining() + "," + img.getSecondaryStaining() + ")";
                        }
            ...
            LocalDate localDate = LocalDate.now();
            try {
                Files.write(new File(outputPath + File.separator + "DTMSCaseOverview_" + localDate + ".txt").toPath(), output.getBytes(), StandardOpenOption.CREATE);
            } catch (IOException ex) {
                ErrorLog.getErrorLog().createLogEntry(2, "Failed writing the exported file"
                        + "");
            }
