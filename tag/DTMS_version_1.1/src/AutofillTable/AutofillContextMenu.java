...
public class AutofillContextMenu<T extends ImageObject> {
    ...
    private void initializeContextMenu() {
        Menu file = new Menu();
        file.setText("File");
        file.getItems().add(EditItem());
        if(this.type.equals("General")){
            file.getItems().add(DeletetItem());
        }
        Menu display = new Menu("Display");
        display.getItems().addAll(FileItem());
        this.menu.getItems().addAll(file,display);
    }
        ...
        item.setOnAction((ActionEvent event) -> {
            for (Object o : this.controller.getAutoFillTable().getSelectedItems()) {
                if (o != null) {
                    ...
                    try {
                        System.out.println(this.cache.getImageServerPath() + t.getFilePath());
    ...
    private MenuItem DeletetItem() {
        ...
        item.setOnAction((ActionEvent event) -> {
            ...
            int result = JOptionPane.showConfirmDialog(null, "Would You Like to delete the " + this.controller.getAutoFillTable().getSelectedItems().size() + "  selected item?", "Remove?", JOptionPane.YES_NO_OPTION);
    ...
    public MenuItem AdditionalFiles(){
        MenuItem item = new MenuItem("Existing additional files");
        item.setOnAction((ActionEvent event) -> {
            for (Object o : this.controller.getAutoFillTable().getSelectedItems()) {
                if (o != null) {
                    ImageObject i = (ImageObject) o;
                    ArrayList<ImageObject> additional = this.cache.getDatabase(type).getAllImagesWithCaseID(i.getCaseID());
                    ArrayList<String> addtionalIDs = new ArrayList<>();
                    additional.forEach((img) -> {
                        addtionalIDs.add(img.getImageID());
                    });
            }
        }});
        return item;
