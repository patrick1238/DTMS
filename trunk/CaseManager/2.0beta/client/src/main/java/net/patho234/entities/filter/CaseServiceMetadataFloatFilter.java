 ...
 * @author rehkind
 */
public class CaseServiceMetadataFloatFilter  extends ClientObjectFilterBase{
    ...
    public CaseServiceMetadataFloatFilter(String metadataFieldName, String serviceType){
     ...
     * @param filterMode
     */
    public CaseServiceMetadataFloatFilter(String metadataFieldName, String serviceType, Integer filterMode){
        this(metadataFieldName, serviceType, filterMode, new Double[]{0., Double.MAX_VALUE, null});
    }
    
    public CaseServiceMetadataFloatFilter(String metadataFieldName, String serviceType, Integer filterMode, Double[] filterValues){
    ...
    public void setEqualValue(Double newEq){
    ...
    public boolean isClientObjectInScope(ClientObjectBase clientObject) {
        ...
        if( clientObject.getClass().equals(ClientCase.class) ){
            ...
            throw new ClassCastException("CaseServiceMetadataFloatFilter only excepts ClientCase and ClientService objects. Filter item class: "+clientObject.getClass().getName());
        }
...
//        Logger.getLogger(getClass()).info("+++ Checking contraint on "+m.getName()+" - values are: '"+searchTerm+"' & '"+m.getData().toString()+"'");
        ...
        Double tmpValueToCheck=null;
        try {
            tmpValueToCheck=(Double)m.getData();
        } catch (ClassCastException e) {
            // can not fullfil constrains cause is no Double / should actually not happen
            Logger.getLogger(getClass()).info("+++ ClientObject is rejected - cannot be casted to floating number ( "+m.getData()+" )");
            return false;
        }
        boolean minCheck;
        boolean maxCheck;
        switch( filterMode.getValue() ){
            ...
            default:
                Logger.getLogger(getClass()).warn("+++ filterMode of CaseServiceMetadataFloatFilter is set to an unknown value ('"+filterMode+"'), will return false by default for all items.");
                return false;
        }
    }
    ...
    static public Integer getModeFromString(String modeAsString){
        switch ( modeAsString ){
            ...
            case "equals":
                return MODE_EQUALS;
            default:
                Logger.getLogger(CaseServiceMetadataFloatFilter.class).warn("String '"+modeAsString+"' cannot be converted into a proper filter mode -> returning NULL");
                return null;
        }
    }
    public String getModeAsString(){
        switch ( filterMode.getValue() ){
            ...
            case MODE_EQUALS:
                return "equals";
            default:
                Logger.getLogger(CaseServiceMetadataFloatFilter.class).warn("Mode '"+filterMode.getValue()+"' cannot be converted into a proper filter mode string -> returning NULL");
                return null;
        }
    }
    ...
    public String toString(){
        return "MetadataFloatFilter@"+getModeAsString()+"?"+minValue+":"+maxValue+":"+eqValue;
    }
}
