 ...
 */
public class ClientObjectOrFilter<T> extends ClientObjectFilterBase<ClientObjectBase>{
    Set<ClientObjectFilterBase<ClientObjectBase>> allFilter = new HashSet<>();
    
    public ClientObjectOrFilter(){}
    public ClientObjectOrFilter( Collection<ClientObjectFilterBase<ClientObjectBase>> filterToAdd ){
    ...
    public boolean isClientObjectInScope(ClientObjectBase clientObject) {
        // if no filters are set or any of them accepts the ClientObject return true
        if( allFilter.isEmpty() ){ return true; }
        return allFilter.stream().anyMatch((filter) -> ( filter.isClientObjectInScope(clientObject) ));
    }
}
