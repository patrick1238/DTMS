...
public class StartScreenController implements Initializable {
    ...
    private void start_register(ActionEvent event) {
        if (ClinicField.getText().length() != 0 && SupplierField.getText().length() != 0 && this.folder != null) {
            ...
            config.replace("open_project",this.folder.getAbsolutePath());
            Stage stage = new Stage();
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/fxml/RegisterScreen.fxml"));
            Parent root = null;
            try {
            ...
            final RegisterScreenController controller = fxmlLoader.getController();
            Scene scene = new Scene(root);
            scene.getStylesheets().add("/styles/Styles.css");
            stage.setTitle(SupplierField.getText() + ", " + ClinicField.getText());
            stage.setScene(scene);
            stage.setOnCloseRequest((final WindowEvent event2) -> {
                controller.close();
            });
            stage.show();
